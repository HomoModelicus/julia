...
# function is_json_syntax(char_str)
#     bool = char_str .== json_syntax_chars()
#     return any(bool)
# end
...
function lex_bool(json_text, start_idx::Int, str_true, str_false)
    ...
    # substr_t = SubString(json_text.s, start_idx:Ltidx)
    # substr_f = SubString(json_text.s, start_idx:Lfidx)
    
    substr_t = json_text[ start_idx:Ltidx ]
    substr_f = json_text[ start_idx:Lfidx ]
    
    # if lowercase(json_text[start_idx:Ltidx]) == "true"
    json_bool = false
    if substr_t == str_true # "true"
        json_bool = true
        #json_text = json_text[(Lt+1):end]
        start_idx += Lt
        success = true
    elseif substr_f == str_false# "false"
    # elseif lowercase(json_text[start_idx:Lfidx]) == "false"
        json_bool = false
        #json_text = json_text[(Lf+1):end]
        start_idx += Lf
        success = true
    end
...
function lex_null(json_text, start_idx::Int, null_str)
    ...
    # if lowercase(json_text[idx]) == "null"
    if substr == null_str # "null"
        success = true
        start_idx += Ln
        # json_text = json_text[(Ln+1):end]
    end
...
function lex_json(json_text::T) where {T <: AbstractString}
    ...
    start_idx::Int = 1
    # tokens = datastructs.Stack{Any}(div(Ls,10)) # heuristics
